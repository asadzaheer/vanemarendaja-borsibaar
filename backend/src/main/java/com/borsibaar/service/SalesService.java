...
public class SalesService {
    ...
    private SaleItemResponseDto processSaleItem(SaleItemRequestDto item, UUID userId, Long organizationId,
        ...
        Inventory inventory = inventoryRepository
                ...
                .orElseThrow(() -> new ResponseStatusException(
                        HttpStatus.NOT_FOUND, "No inventory found for product: " + item.productId()));

        // Check if we have enough stock
        BigDecimal oldQuantity = inventory.getQuantity();
        if (oldQuantity.compareTo(item.quantity()) < 0) {
            throw new ResponseStatusException(
                    HttpStatus.BAD_REQUEST, "Not enough stock available");
        }

        // Calculate pricing
        BigDecimal priceBeforeSale = pricingService.calculateCurrentPrice(inventory, product);
        BigDecimal totalPrice = pricingService.calculateTotalPrice(priceBeforeSale, item.quantity());
        Category category = product.getCategory();
        BigDecimal priceAfterSale = pricingService.calculatePriceAfterSale(priceBeforeSale, product, category);
